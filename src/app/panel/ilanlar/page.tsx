...
import { BulkSelectionControls } from "@/app/panel/ilanlar/BulkSelectionControls";
import { DeleteProductForm } from "@/app/panel/ilanlar/DeleteProductForm";
import { QuickStatusForm } from "@/app/panel/ilanlar/QuickStatusForm";
...
export default async function ProductsPage({ searchParams }: ProductsPageProps) {
  ...
  const buildPageHref = (targetPage: number) => {
    ...
    return `/panel/ilanlar?${next.toString()}`;
  };
  ...
  return (
    <section>
      <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        ...
        {canEdit ? (
          <Link href="/panel/ilanlar/yeni" className="flex h-11 items-center justify-center rounded-xl bg-neutral-950 px-5 text-sm font-semibold text-white transition hover:bg-neutral-800">
            Yeni ilan ekle
          </Link>
        ) : null}
      </div>
      ...
      <form className="mt-6 grid gap-3 rounded-[22px] bg-white p-4 shadow-sm ring-1 ring-black/[0.05] md:grid-cols-2 xl:grid-cols-[1.5fr_repeat(5,minmax(0,1fr))_auto]">
        ...
        <div className="flex gap-2">
          ...
          <Link href="/panel/ilanlar" className="flex h-11 items-center justify-center rounded-xl border border-neutral-200 px-3 text-sm font-medium text-neutral-600">Temizle</Link>
        </div>
      </form>
      ...
      <div className="mt-6 overflow-hidden rounded-[24px] bg-white shadow-sm ring-1 ring-black/[0.05]">
        ...
        {visibleProducts.length > 0 ? (
              ...
              return (
                <article key={product.id} className="grid gap-4 px-4 py-4 transition hover:bg-neutral-50/70 sm:px-5 xl:grid-cols-[28px_minmax(240px,1.4fr)_140px_140px_125px_105px_110px_120px_170px] xl:items-center xl:gap-3">
                  ...
                  <div className="flex flex-wrap gap-2 xl:justify-end">
                    <Link href={`/panel/ilanlar/${product.id}`} className="rounded-lg bg-neutral-950 px-3 py-2 text-[11px] font-semibold text-white">Detay</Link>
                    ...
                    {canEdit ? <Link href={`/panel/ilanlar/${product.id}/duzenle`} className="rounded-lg border border-neutral-200 px-3 py-2 text-[11px] font-medium text-neutral-700">Düzenle</Link> : null}
          ...
          <div className="px-5 py-16 text-center">
            ...
            <Link href="/panel/ilanlar" className="mt-3 inline-flex text-sm font-semibold text-neutral-950 underline">Filtreleri temizle</Link>
          </div>
        )}
      </div>
