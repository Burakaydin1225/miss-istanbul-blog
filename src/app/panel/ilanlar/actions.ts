...
function refreshProductPages(
  ...
  revalidatePath("/panel/ilanlar");
  revalidatePath("/panel/ilanlar/yeni");

  for (const region of productRegions) {
    revalidatePath(`/ilce/${region.slug}`);
  }

  if (slug) {
    revalidatePath(`/ilan/${slug}`);
  }

  if (productId) {
    revalidatePath(
      `/panel/ilanlar/${productId}`,
    );

    revalidatePath(
      `/panel/ilanlar/${productId}/duzenle`,
    );
  }
}
