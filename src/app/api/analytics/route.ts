...
export async function POST(request: Request) {
  try {
    ...
    const placementId = cleanString(
      body.placementId,
      100,
    );

    const placementEvent =
      eventType === AnalyticsEventType.PLACEMENT_IMPRESSION ||
      eventType === AnalyticsEventType.PLACEMENT_CLICK;

    if (placementEvent && !placementId) {
      return NextResponse.json(
        { error: "Reklam konumu olaylarında konum kimliği zorunludur." },
        { status: 400 },
      );
    }
    ...
    if (placementId) {
      ...
      if (!placement) {
        return NextResponse.json(
          { error: "Reklam konumu bulunamadı." },
          { status: 404 },
        );
      }

      if (productId && placement.productId !== productId) {
        return NextResponse.json(
          { error: "İlan ve reklam konumu eşleşmiyor." },
          { status: 400 },
        );
      }
    }
    ...
    const activeSessionProductId = productEvent ? productId : null;
    ...
    if (
      ...
      activeSessionUpdateData.productId = activeSessionProductId;
    }
